...
async function handleCapture(rect, tab) {
  try {
    ...
    // Wait for the page to repaint without the selection overlay before screenshotting.
    // The content script already waits 150ms before sending this message, but we add
    // a small additional buffer here for safety.
    await sleep(100);
    ...
    if (!apiKey)
      throw new Error(`No API key for ${provider}. Open the extension settings.`);
  ...
  for (const model of visionModels) {
    ...
    try {
      ...
      if (e.message.includes("API key") || e.message.includes("rate limit"))
        throw e;
      console.warn("[FactCheck] HF vision failed with model:", model, e.message);
    }
  }
  ...
  for (const model of HF_VISION_MODELS) {
    try {
      ...
      if (text && text.length > 10) {
        console.log("[FactCheck] HF OCR success with model:", model, "- chars:", text.length);
        return text;
      }
    } catch (e) {
...
async function callHuggingFaceText(text, apiKey, preferredModel) {
  ...
  for (const model of models) {
    try {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        console.warn("[FactCheck] HF text model", model, "failed:", res.status, err?.error?.message);
        if (res.status === 401) throw new Error("Invalid HuggingFace API key.");
        if (res.status === 429)
          throw new Error("HuggingFace rate limit hit. Please wait and try again.");
        continue;
...
async function assertOk(res, providerName) {
  ...
  if (res.status === 429)
    throw new Error(`${providerName} rate limit reached. Please wait and try again.`);
